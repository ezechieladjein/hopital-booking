...
export default function SecretaryDashboard() {
  ...
  const [selectedDateSlots, setSelectedDateSlots] = useState(new Date().toISOString().split('T')[0]);
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      if (resCatalog.success) setSpecialities(resCatalog.data.specialities || resCatalog.data || []);
    } catch (err) {
  ...
  const fetchDoctorData = (doctorId, date) => {
    fetch(`${API_BASE_URL}/doctors/${doctorId}/slots?date=${date}`)
      .then(res => res.json())
      .then(data => { if (data.success) setDoctorSlots(data.data || []); });

    fetch(`${API_BASE_URL}/secretary/doctors/${doctorId}/unavailabilities`)
      .then(res => res.json())
      .then(data => { if (data.success) setDoctorUnavailabilities(data.data || []); });

    fetch(`${API_BASE_URL}/secretary/doctors/${doctorId}/availabilities`)
      .then(res => res.json())
      .then(data => { if (data.success) setWeeklyAvailabilities(data.data || []); });
  };
  ...
  const filteredAppointments = useMemo(() => {
    return appointments.filter(appt => {
      const patientName = `${appt.patient?.nom} ${appt.patient?.prenom}`.toLowerCase();
      const doctorName = `dr. ${appt.slot?.doctor?.nom}`.toLowerCase();
      const searchMatch = patientName.includes(searchAppt.toLowerCase()) || 
                          doctorName.includes(searchAppt.toLowerCase()) || 
                          appt.id.toString().includes(searchAppt);

      const statusMatch = statusFilterAppt === 'ALL' || appt.status === statusFilterAppt;
      const insuranceMatch = insuranceFilterAppt === 'ALL' || 
        (insuranceFilterAppt === 'WITH' && appt.has_insurance) ||
        (insuranceFilterAppt === 'WITHOUT' && !appt.has_insurance);

      const dateMatch = !dateFilterAppt || appt.slot?.date_consultation === dateFilterAppt;

      return searchMatch && statusMatch && insuranceMatch && dateMatch;
    });
  }, [appointments, searchAppt, statusFilterAppt, insuranceFilterAppt, dateFilterAppt]);

  const filteredDoctors = useMemo(() => {
    return doctors.filter(doc => {
      const fullName = `${doc.nom} ${doc.prenom}`.toLowerCase();
      const searchMatch = fullName.includes(searchDoctor.toLowerCase());
      const specMatch = specialityFilter === 'ALL' || String(doc.speciality_id) === String(specialityFilter);
  ...
  const handleUnblock = (unavailId) => {
    if (!window.confirm("Voulez-vous débloquer cette période et réouvrir les créneaux ?")) return;
  ...
  const handleValidateInsurance = () => {
    fetch(`${API_BASE_URL}/secretary/validate-insurance`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ appointment_id: selectedAppointment.id, insurance_coverage_rate: parseInt(coverageRate) })
    }).then(() => { setSelectedAppointment(null); setCoverageRate(''); fetchData(); });
  };
  ...
  const handleRejectInsurance = () => {
    fetch(`${API_BASE_URL}/secretary/reject-insurance`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ appointment_id: selectedAppointment.id, reason: cancellationReason })
    }).then(() => { setSelectedAppointment(null); setCancellationReason(''); fetchData(); });
  };
  ...
  const handleSaveAvailabilities = async () => {
    ...
    try {
      const res = await fetch(`${API_BASE_URL}/secretary/doctors/${selectedDoctor.id}/availabilities`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  if (loading) return <div className="p-8 text-center text-gray-500 font-['Poppins']">Chargement du dashboard secrétariat...</div>;
  ...
  return (
    <div className="bg-gray-50 min-h-screen p-6 font-['Poppins'] text-gray-800">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8 bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
        <div>
          <h1 className="text-2xl font-bold text-[#0D1B3D]">Espace Secrétariat</h1>
          <p className="text-xs text-gray-400 mt-1">Gestion des rendez-vous et des plannings</p>
        </div>
            ...
            <table className="w-full text-left border-collapse">
              ...
              <tbody className="divide-y divide-gray-100 text-xs">
                {filteredAppointments.length === 0 ? (
                  <tr>
                    <td colSpan="8" className="text-center py-8 text-gray-400">Aucun rendez-vous trouvé.</td>
                  </tr>
                ) : (
                  filteredAppointments.map((appt) => {
                    ...
                    return (
                      <tr key={appt.id} className="hover:bg-gray-50/60 transition">
                        <td className="py-3.5 px-4 font-bold text-gray-600">{formattedCreated}</td>
                        <td className="py-3.5 px-4 font-bold text-[#0D1B3D]">{appt.patient?.nom} {appt.patient?.prenom}</td>
                        <td className="py-3.5 px-4">Dr. {appt.slot?.doctor?.nom}</td>
                        ...
                        <td className="py-3.5 px-4">
                          ...
                          ) : <span className="text-gray-400 italic">Sans</span>}
                        </td>
                        ...
                        <td className="py-3.5 px-4 text-center">
                          {appt.has_insurance && appt.status === 'EN_ATTENTE_VALIDATION' ? (
                            ...
                            <span className="text-gray-300 text-[10px] italic">Aucune action</span>
                          )}
                        </td>
                      </tr>
                    );
      ...
      {activeTab === 'doctors' && (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="bg-white p-5 rounded-2xl shadow-sm border border-gray-100 space-y-4">
            <h2 className="text-sm font-bold text-[#0D1B3D]">Sélectionner un médecin</h2>
              ...
              {filteredDoctors.map(doc => (
                <div
                  ...
                  className={`p-3 rounded-xl cursor-pointer transition flex justify-between items-center ${
                    selectedDoctor?.id === doc.id ? 'bg-blue-50 border border-blue-200' : 'hover:bg-gray-50'
                  }`}
                >
                  <div>
                    <p className="font-bold text-xs text-[#0D1B3D]">Dr. {doc.nom} {doc.prenom}</p>
                    <p className="text-[10px] text-blue-600 font-medium">{doc.speciality?.nom || 'Spécialité N/A'}</p>
                  </div>
                  ...
                  <div className="bg-indigo-50/40 p-5 rounded-2xl border border-indigo-100 space-y-4">
                    ...
                    <div className="space-y-2">
                      {weeklyAvailabilities.map((avail, idx) => (
                        <div key={idx} className="flex items-center gap-2 bg-white p-2 rounded-xl border border-indigo-50">
                          <select
                    ...
                    <div className="flex flex-col sm:flex-row gap-3 items-end">
                      <div className="flex-1">
                        <label className="block text-[10px] font-bold text-gray-500 mb-1">Du</label>
                        <input
                          type="date"
                          value={startDateGen}
                      ...
                      <div className="flex-1">
                        <label className="block text-[10px] font-bold text-gray-500 mb-1">Au</label>
                        <input
                          type="date"
                          value={endDateGen}
                    ...
                    {genMessage && (
                      <p className={`text-xs font-bold p-2 rounded-lg ${
                        genMessage.type === 'success' ? 'bg-emerald-100 text-emerald-800' : 
                        genMessage.type === 'danger' ? 'bg-red-100 text-red-800' : 'bg-blue-100 text-blue-800'
                      }`}>
                        {genMessage.text}
                      </p>
                    )}
                ...
                <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 space-y-5">
                  <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-4 pb-4 border-b">
                    <div>
                      ...
                      <p className="text-xs text-gray-400">{selectedDoctor.speciality?.nom}</p>
                    </div>
                    ...
                    {doctorSlots.length === 0 ? (
                      ...
                      doctorSlots.map(slot => {
                        ...
                        return (
                          <div
                            key={slot.id}
                            onClick={() => isAvailable && handleToggleSlotSelection(slot.id)}
                            className={`p-3 rounded-xl border text-center transition cursor-pointer relative ${
                              isSelected ? 'ring-2 ring-red-500 border-transparent bg-red-100/50' : ''
                            } ${
                              isAvailable 
                          ...
                          >
                            <p className="font-bold text-xs">{slot.start_time?.substring(0, 5)} - {slot.end_time?.substring(0, 5)}</p>
                        ...
                        {doctorUnavailabilities.length === 0 ? (
                          ...
                          doctorUnavailabilities.map((u) => (
                            <tr key={u.id}>
                              ...
                              <td className="py-3 px-3 text-amber-700 font-medium">{u.type}</td>
                              <td className="py-3 px-3">{u.reason || 'Non précisé'}</td>
                              <td className="py-3 px-3 text-center">
                                <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold ${
                                  u.status === 'ACTIF' ? 'bg-red-100 text-red-700' : 'bg-gray-100 text-gray-500'
                                }`}>
          ...
          <div className="bg-white rounded-2xl p-6 max-w-md w-full shadow-xl space-y-4">
            ...
            <div className="p-3 bg-gray-50 border border-gray-100 rounded-xl space-y-2">
              <div className="text-xs">
                <span className="font-semibold text-gray-600">Assurance : </span>
                <span className="font-bold text-[#0D1B3D]">{selectedAppointment.insurance_name || 'N/A'}</span>
                {selectedAppointment.insurance_policy_number && (
                  <span className="block text-[11px] text-gray-500">N° Police: {selectedAppointment.insurance_policy_number}</span>
                )}
              </div>

              {selectedAppointment.insurance_document_path ? (
                <a
                  href={getInsuranceDocumentUrl(selectedAppointment.insurance_document_path)}
                  target="_blank"
            ...
            {!isRejecting ? (
              <div>
                <label className="block text-[11px] font-semibold text-gray-500 mb-1">Taux de couverture (%)</label>
                <input
                  type="number"
                  placeholder="ex: 80"
                  value={coverageRate}
              ...
              <div>
                <label className="block text-[11px] font-semibold text-gray-500 mb-1">Motif du refus</label>
                <input
                  type="text"
            ...
            <div className="flex justify-end gap-2 pt-2 border-t">
              <button onClick={() => setSelectedAppointment(null)} className="px-4 py-2 text-xs font-bold text-gray-500 hover:text-gray-700">
                Annuler
              </button>
              <button
                onClick={!isRejecting ? handleValidateInsurance : handleRejectInsurance}
